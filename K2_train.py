# Modified K2_train.py for K2 with added joblib.dumps
...
try:
    # Step 1: Load & Clean
    df = pd.read_csv('/kaggle/input/exoplanetslaplaceecliptic/K2edited.csv')  # Adjust path to your K2 dataset CSV
    identifiers = df[['pl_name', 'hostname', 'disposition']].copy()  # Save for predictions
    ...
    le = LabelEncoder()
    df['target'] = le.fit_transform(df['disposition'])
    joblib.dump(le, '/kaggle/working/k2_le.pkl')
    df = df.drop('disposition', axis=1)
    logger.info(f"Shape: {df.shape}, Target dist: \n{df['target'].value_counts(normalize=True)}")

    # Step 2: Handle Asymmetric Errors
    # No _err1/_err2 in K2; symerr present but treated as numeric. Skipped averaging.
    logger.info("No asymmetric error columns to average in K2.")
    ...
    joblib.dump(num_cols, '/kaggle/working/k2_num_cols.pkl')
    joblib.dump(valid_cat_cols, '/kaggle/working/k2_valid_cat_cols.pkl')
    ...
    try:
        if num_cols:
            ...
            joblib.dump(imputer_num, '/kaggle/working/k2_imputer_num.pkl')
        ...
        if cat_cols:
            ...
            if valid_cat_cols:
                ...
                joblib.dump(imputer_cat, '/kaggle/working/k2_imputer_cat.pkl')
    ...
    try:
        ...
        joblib.dump(columns_after_encoding, '/kaggle/working/k2_columns_after_encoding.pkl')
    except Exception as e:
    ...
    try:
        # No direct 'pl_ratror'; approximate using pl_rade / st_rad (normalized units)
        ...
        joblib.dump(clip_num_cols, '/kaggle/working/k2_clip_num_cols.pkl')
    except Exception as e:
    ...
    try:
        ...
        joblib.dump(clip_bounds, '/kaggle/working/k2_clip_bounds.pkl')
        ...
        joblib.dump(to_drop, '/kaggle/working/k2_to_drop.pkl')
        ...
        joblib.dump(top_features, '/kaggle/working/k2_top_features.pkl')
        X = df[top_features]
    ...
    try:
        ...
        joblib.dump(scaler, '/kaggle/working/k2_scaler.pkl')
    ...
    try:
        ...
        pred_df['disposition_pred'] = le.inverse_transform(y_pred)
        pred_df.to_csv('/kaggle/working/predictions_k2.csv', index=False)
        logger.info("Predictions saved to /kaggle/working/predictions_k2.csv")
        ...
        plt.savefig('/kaggle/working/importances_k2_correct.png')
        plt.show()
        
        # Save model
        joblib.dump(model, '/kaggle/working/K2_xgb_model.pkl')
        logger.info("Model saved to /kaggle/working/K2_xgb_model.pkl")
    except Exception as e:
